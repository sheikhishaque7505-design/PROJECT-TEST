// src/SmartCity3D.jsx
// GLB-FREE — sab buildings code se. No external files needed.
...
const BUILDING_INFO = {
  ...
  greenhouse: { title: "Smart Greenhouse", type: "AGRICULTURE", icon: "🌱", desc: "Climate-controlled farming building with automated irrigation.", stats: [["Beds","24"],["Climate","Auto"],["Irrigation","Drip"],["Yield","+220%"]] },
...
export const LOCATIONS = {
  ...
  verticalFarm: { key: "verticalFarm", label: "Vertical Farm Towers", icon: "🌿", type: "AGRICULTURE", position: [2900, 5, -2400], camHeight: 480, camDistance: 460, cameras: [{ name: "Top View", top: true }] },
  greenhouse: { key: "greenhouse", label: "Smart Greenhouse", icon: "🌱", type: "AGRICULTURE", position: [3400, 5, -1800], camHeight: 400, camDistance: 400, cameras: [{ name: "Top View", top: true }] },
  smartHomes: { key: "smartHomes", label: "Smart Homes", icon: "🏠", type: "RESIDENTIAL", position: [-1400, 5, -1500], camHeight: 320, camDistance: 340, cameras: [{ name: "Top View", top: true }] },
};
...
export const LOCATION_LIVE_DATA = {
  ...
  greenhouse: { desc: "Climate-controlled farming", stats: [["Beds","24"],["Climate","Auto"],["Drip","Yes"],["Yield","+220%"]] },
...
const SmartCity3D = forwardRef((props, ref) => {
  ...
  const s = useRef({
    ...
    trucks: {}, people: [], cityCars: [], intersectionCars: [],
    ...
    verticalFarms: [], growLights: [], smartHomeSigns: [], rooftopTurbines: [],
    greenhouseCrops: [], greenhouseCropData: [],
  }).current;
  ...
  // src/SmartCity3D.jsx — PART 2 (Part 1 ke NEECHE paste karo, usi file mein)
// Yeh Part 1 ke `setDayNight` function ke baad aata hai
  ...
  useEffect(() => {
    ...
    // ============================================================
    // SMART HOME — with rooftop wind turbine (as per user's picture)
    // Windows, door, solar panel, rooftop turbine
    // ============================================================
    function buildSmartHome(cx, cz, name, wallColor, roofColor, rot = 0) {
      const g = new THREE.Group(); g.position.set(cx, 5, cz); g.rotation.y = rot;

      // Foundation
      const base = new THREE.Mesh(new THREE.BoxGeometry(170, 4, 170), mat(0x5a5a5a, 0.9));
      base.position.y = 2; g.add(base);

      // Main body
      const bodyMat = mat(wallColor, 0.75, 0.05);
      const body = new THREE.Mesh(new THREE.BoxGeometry(130, 70, 120), bodyMat);
      body.position.y = 39; g.add(body);

      // Roof (gable)
      const roofShape = new THREE.Shape();
      roofShape.moveTo(-75, 0); roofShape.lineTo(0, 40); roofShape.lineTo(75, 0); roofShape.lineTo(-75, 0);
      const roofGeo = new THREE.ExtrudeGeometry(roofShape, { depth: 130, bevelEnabled: false });
      const roofMesh = new THREE.Mesh(roofGeo, mat(roofColor, 0.6, 0.15));
      roofMesh.rotation.y = 0; roofMesh.position.set(0, 74, -65); g.add(roofMesh);

      // Door
      const door = new THREE.Mesh(new THREE.BoxGeometry(24, 44, 3), mat(0x5a2a1a, 0.6, 0.05));
      door.position.set(0, 27, 61); g.add(door);
      const doorHandle = new THREE.Mesh(new THREE.SphereGeometry(1.2, 8, 8), mat(0xd4af37, 0.3, 0.9));
      doorHandle.position.set(9, 27, 63); g.add(doorHandle);
      // Door frame
      const doorFrame = new THREE.Mesh(new THREE.BoxGeometry(28, 48, 2), mat(0x3a1a0a, 0.7));
      doorFrame.position.set(0, 26, 60); g.add(doorFrame);

      // Windows (2 front, 2 side)
      const winMat = new THREE.MeshStandardMaterial({ color: 0x9adfff, emissive: 0x66aaff, emissiveIntensity: 1.5, transparent: true, opacity: 0.75, metalness: 0.4, roughness: 0.15 });
      const winFrameMat = mat(0x3a2a1a, 0.7);
      // Front windows
      for (const wx of [-42, 42]) {
        const frame = new THREE.Mesh(new THREE.BoxGeometry(28, 26, 2), winFrameMat);
        frame.position.set(wx, 48, 61); g.add(frame);
        const win = new THREE.Mesh(new THREE.BoxGeometry(24, 22, 1.5), winMat);
        win.position.set(wx, 48, 62); g.add(win);
        // Window cross
        const crossH = new THREE.Mesh(new THREE.BoxGeometry(24, 0.6, 1.7), winFrameMat);
        crossH.position.set(wx, 48, 62.2); g.add(crossH);
        const crossV = new THREE.Mesh(new THREE.BoxGeometry(0.6, 22, 1.7), winFrameMat);
        crossV.position.set(wx, 48, 62.2); g.add(crossV);
      }
      // Side windows
      for (const side of [-1, 1]) {
        for (const wz of [-30, 30]) {
          const frame = new THREE.Mesh(new THREE.BoxGeometry(2, 24, 24), winFrameMat);
          frame.position.set(side * 66, 42, wz); g.add(frame);
          const win = new THREE.Mesh(new THREE.BoxGeometry(1.5, 20, 20), winMat);
          win.position.set(side * 67, 42, wz); g.add(win);
        }
      }

      // Rooftop solar panel
      const solar = new THREE.Mesh(new THREE.BoxGeometry(100, 2, 70), new THREE.MeshStandardMaterial({ color: 0x082c4b, roughness: 0.18, metalness: 0.8, emissive: 0x0a3a5a, emissiveIntensity: 0.8 }));
      solar.rotation.x = -0.15; solar.position.set(0, 76, -10); g.add(solar);

      // Rooftop WIND TURBINE (like user's picture)
      const turbineGroup = new THREE.Group();
      turbineGroup.position.set(0, 120, -30);
      // Pole
      const turbinePole = new THREE.Mesh(new THREE.CylinderGeometry(1.2, 1.6, 30, 8), mat(0xeeeeee, 0.5, 0.3));
      turbinePole.position.y = 15; turbineGroup.add(turbinePole);
      // Hub (small)
      const hub = new THREE.Mesh(new THREE.SphereGeometry(2.5, 10, 10), mat(0xcccccc, 0.4, 0.5));
      hub.position.y = 30; turbineGroup.add(hub);
      // Blades (rotating)
      const blades = new THREE.Group(); blades.position.y = 30;
      for (let i = 0; i < 3; i++) {
        const blade = new THREE.Mesh(new THREE.BoxGeometry(0.8, 18, 0.4), mat(0xffffff, 0.5, 0.3));
        blade.position.y = 9;
      ...
      sign.position.set(0, 92, 62); g.add(sign);
    ...
    // ============================================================
    // VERTICAL FARM TOWER — with REAL-TIME growing crops
    // ============================================================
    function buildVerticalFarm(cx, cz, count = 6) {
      for (let i = 0; i < count; i++) {
        ...
        for (let l = 0; l < levels; l++) {
          ...
          // Crops at this level — 8 plants, will grow over time
          const levelCrops = [];
          for (let k = 0; k < 8; k++) {
            ...
            plant.position.set(px, y + 4, pz);

            // Stem
            const stem = new THREE.Mesh(new THREE.CylinderGeometry(0.4, 0.5, 4, 6), mat(0x2a7a2a, 0.9));
            stem.position.y = 2; plant.add(stem);

            // Leaves (will scale up over time)
            const leafMat = new THREE.MeshStandardMaterial({ color: 0x3ab54a, roughness: 0.85, emissive: 0x0a3a1a, emissiveIntensity: 0.5 });
            const leaves = new THREE.Group();
            for (let lf = 0; lf < 4; lf++) {
              const leaf = new THREE.Mesh(new THREE.SphereGeometry(2.5, 6, 5), leafMat);
              leaf.position.set(Math.cos(lf * 1.57) * 2.5, 3 + lf * 0.5, Math.sin(lf * 1.57) * 2.5);
              leaf.scale.set(1, 0.6, 1);
              leaves.add(leaf);
            }
            plant.add(leaves);

            // Fruit/vegetable (grows + changes color)
            const fruit = new THREE.Mesh(new THREE.SphereGeometry(1.5, 8, 8), new THREE.MeshStandardMaterial({ color: 0x2a8a2a, emissive: 0x0a3a1a, emissiveIntensity: 0.3, roughness: 0.6 }));
            fruit.position.y = 5; plant.add(fruit);

            g.add(plant);
            levelCrops.push({ plant, stem, leaves, fruit, grow: Math.random() * Math.PI * 2, level: l });
          }
          cropLevels.push(levelCrops);
        }
        ...
        s.verticalFarms.push({ g, beacon, topLED, cropLevels });
      }
    }

    // ============================================================
    // GREENHOUSE BUILDING — crop growth in real-time
    // ============================================================
    function buildGreenhouse(cx, cz) {
      const g = new THREE.Group(); g.position.set(cx, 5, cz);

      // Pad
      const pad = new THREE.Mesh(new THREE.BoxGeometry(500, 2, 400), mat(0x3a4a3a, 0.95));
      pad.position.y = 1; g.add(pad);

      // Glass structure
      const glassMatH = new THREE.MeshStandardMaterial({ color: 0x9adfff, transparent: true, opacity: 0.28, metalness: 0.3, roughness: 0.1, side: THREE.DoubleSide });
      const wallA = new THREE.Mesh(new THREE.BoxGeometry(500, 90, 3), glassMatH);
      wallA.position.set(0, 45, 200); g.add(wallA);
      const wallB = wallA.clone(); wallB.position.z = -200; g.add(wallB);
      const wallC = new THREE.Mesh(new THREE.BoxGeometry(3, 90, 400), glassMatH);
      wallC.position.set(-250, 45, 0); g.add(wallC);
      const wallD = wallC.clone(); wallD.position.x = 250; g.add(wallD);

      // Roof (arched)
      const roofGeo = new THREE.CylinderGeometry(0, 0, 0, 0, 0);
      const roofMatH = new THREE.MeshStandardMaterial({ color: 0x9adfff, transparent: true, opacity: 0.35, metalness: 0.4, roughness: 0.15 });
      for (let i = 0; i < 6; i++) {
        const arch = new THREE.Mesh(new THREE.CylinderGeometry(200, 200, 2, 24, 1, true, 0, Math.PI), roofMatH);
        arch.rotation.z = Math.PI / 2;
        arch.rotation.y = Math.PI / 2;
        arch.position.set(-250 + i * 100, 90, 0);
        arch.scale.set(1, 0.5, 1);
        g.add(arch);
      }

      // Framework
      const frameMat = mat(0x8a8a8a, 0.4, 0.7);
      for (let i = 0; i < 6; i++) {
        const post = new THREE.Mesh(new THREE.BoxGeometry(3, 90, 3), frameMat);
        post.position.set(-250 + i * 100, 45, 200); g.add(post);
        const post2 = post.clone(); post2.position.z = -200; g.add(post2);
      }
      ...
      for (let r = 0; r < 4; r++) {
        for (let c = 0; c < 6; c++) {
          ...
          // Raised bed
          const bed = new THREE.Mesh(new THREE.BoxGeometry(70, 6, 70), mat(0x4a2a1a, 0.9));
          bed.position.set(bx, 5, bz); g.add(bed);

          // Soil
          const soil = new THREE.Mesh(new THREE.BoxGeometry(66, 2, 66), mat(0x2a1a0a, 0.95));
          soil.position.set(bx, 8.5, bz); g.add(soil);

          // Rows of plants — 3x3 grid on each bed
          const bedCrops = [];
          for (let pr = 0; pr < 3; pr++) {
            for (let pc = 0; pc < 3; pc++) {
              const px = bx - 22 + pc * 22;
              const pz = bz - 22 + pr * 22;

              const plant = new THREE.Group();
              plant.position.set(px, 10, pz);

              // Stem
              const stem = new THREE.Mesh(new THREE.CylinderGeometry(0.4, 0.5, 5, 6), mat(0x2a7a2a, 0.9));
              stem.position.y = 2.5; plant.add(stem);

              // Leaves
              const leafMat2 = new THREE.MeshStandardMaterial({ color: 0x4ac84a, roughness: 0.8, emissive: 0x0a4a0a, emissiveIntensity: 0.5 });
              const leaves = new THREE.Group();
              for (let lf = 0; lf < 5; lf++) {
                const leaf = new THREE.Mesh(new THREE.SphereGeometry(2.8, 6, 5), leafMat2);
                leaf.position.set(Math.cos(lf * 1.25) * 3, 3.5 + lf * 0.4, Math.sin(lf * 1.25) * 3);
                leaf.scale.set(1, 0.5, 1);
                leaves.add(leaf);
              }
              plant.add(leaves);

              // Produce (colorful when mature)
              const fruit = new THREE.Mesh(new THREE.SphereGeometry(1.8, 8, 8), new THREE.MeshStandardMaterial({ color: 0x2a8a2a, emissive: 0x0a3a1a, emissiveIntensity: 0.3, roughness: 0.5 }));
              fruit.position.y = 6; plant.add(fruit);

              g.add(plant);
              bedCrops.push({ plant, stem, leaves, fruit, grow: Math.random() * Math.PI * 2 });
            }
          }
          cropBeds.push(bedCrops);
        }
      }

      // Grow lights on ceiling
      const glowMatCeil = new THREE.MeshStandardMaterial({ color: 0xff9adf, emissive: 0xff9adf, emissiveIntensity: 3.5 });
      s.growLights.push(glowMatCeil);
      for (let i = 0; i < 6; i++) {
        const light = new THREE.Mesh(new THREE.BoxGeometry(60, 2, 2), glowMatCeil);
        light.position.set(-200 + i * 80, 85, 0); g.add(light);
      }

      // Entrance sign
      board("SMART GREENHOUSE", cx, 5, cz + 220, 320, 18, 0x2a8a4a);

      scene.add(g);
      s.greenhouseCrops = cropBeds;
      clickable.push({ object: g, type: "greenhouse", name: "Smart Greenhouse" });
    }

    // ============================================================
    // GENERIC BUILDING BUILDER
    // ============================================================
    function makeBuilding(x, z, w, d, h, bodyColor, roofColor, type, name, hasBorder = true, borderColor = 0x22cfff, hasWindows = true) {
      const g = new THREE.Group(); g.position.set(x, 5, z);
      const bMat = mat(bodyColor, 0.6, 0.3);
      const body = new THREE.Mesh(new THREE.BoxGeometry(w, h, d), bMat);
      body.position.y = h / 2; g.add(body);
      const roof = new THREE.Mesh(new THREE.BoxGeometry(w + 4, 6, d + 4), mat(roofColor || 0x2a2a35, 0.6, 0.4));
      roof.position.y = h + 3; g.add(roof);
      if (hasWindows) {
        const winMat = new THREE.MeshStandardMaterial({ color: 0x0a2a3a, emissive: 0x66ccff, emissiveIntensity: 0.7, metalness: 0.5, roughness: 0.2 });
        const floors = Math.min(10, Math.max(2, Math.floor(h / 22)));
      ...
      // Door
      const door = new THREE.Mesh(new THREE.BoxGeometry(Math.min(w * 0.3, 24), Math.min(14, h * 0.2), 3), mat(0x0a0a0a, 0.5, 0.4));
      ...
      if (hasBorder) buildingBorder(x, z, w + 20, d + 20, borderColor);
      return g;
    }

    // ===== ALL BUILDINGS (GLB replacements) =====
    makeBuilding(-600, -600, 260, 240, 85, 0xa83a3a, 0x5a2a2a, "school", "American High School", true, 0x1a5490);
    makeBuilding(600, -600, 240, 220, 95, 0xf0f0f0, 0xc0392b, "hospital", "Smart Hospital", true, 0xc0392b);
    makeBuilding(600, 600, 180, 180, 190, 0x6a3a8a, 0x4a2a6a, "bank", "State Bank", true, 0x8e44ad);
    makeBuilding(1800, -600, 320, 300, 45, 0x8a6a3a, 0x5a4a2a, "farm", "Smart Eco Farm", true, 0x27ae60);
    makeBuilding(600, 1800, 300, 260, 65, 0xd4a017, 0x8a6a17, "newHall", "Liverpool Event Hall", true, 0xd4a017);
    makeBuilding(1800, 1750, 260, 200, 40, 0xc0392b, 0x8a2a1a, "carWash", "Gas Station", true, 0xc0392b);
    makeBuilding(1800, 600, 240, 220, 75, 0x2a6a4a, 0x1a4a2a, "sewageCompany", "Sewage and Gas Co.", true, 0x2ecc71);
    makeBuilding(2200, 600, 200, 180, 85, 0x4a5a6a, 0x2a3a4a, "sewageCompany", "Old Office", true, 0x34495e);
    makeBuilding(-1800, 1800, 340, 280, 65, 0xc98a3a, 0x8a5a2a, "cultureCenter", "Culture Center", true, 0xf39c12);
    makeBuilding(-1600, 800, 280, 240, 95, 0xf1c40f, 0xb8960a, "powerCompany", "City Power Supply", true, 0xf1c40f);
    makeBuilding(-3900, -2400, 320, 280, 150, 0x3a8a4a, 0x2a5a3a, "scifi9", "Sci-Fi Building 9", true, 0x66ff99);
    makeBuilding(-3600, -800, 220, 220, 230, 0x22a5b8, 0x15808a, "beautifulTower", "Beautiful Tower", true, 0x22cfff);
    makeBuilding(-3600, 800, 320, 280, 150, 0xd94ad9, 0x8a2a8a, "scifi10", "Sci-Fi Building 10", true, 0xff66dd);
    makeBuilding(2900, 1200, 360, 320, 110, 0x22a5b8, 0x15808a, "modernBuilding", "Modern Building Complex", true, 0x22cfff);
    makeBuilding(2900, 2400, 500, 300, 190, 0x3a3a5a, 0x1a1a2a, "skyline", "Downtown Skyline", true, 0xff66dd);
    makeBuilding(-2900, 1600, 320, 280, 95, 0xffd15a, 0xa88a2a, "greatHall", "Great Hall", true, 0xffd15a);
    makeBuilding(1000, -1200, 140, 140, 65, 0xf1c40f, 0xa8900a, "bank", "ATM Branch", true, 0xf1c40f);
    makeBuilding(-2400, 2200, 200, 200, 75, 0x6cd4a0, 0x3a8a6a, "modernBuilding", "Residential Pair", true, 0x6cd4a0);
    makeBuilding(2400, -1800, 260, 240, 85, 0xf39c12, 0xa86a0a, "commercial", "Commercial Hub", true, 0xf39c12);
    makeBuilding(-2900, 2400, 320, 280, 110, 0x22a5b8, 0x15808a, "modernBuilding", "Modern West Block", true, 0x22cfff);
    makeBuilding(900, 1400, 100, 100, 25, 0x2ecc71, 0x1a8a4a, "wasteBin", "Trash Props", false, 0x2ecc71);

    // ===== SMART HOMES with ROOFTOP TURBINES (like user's picture) =====
    const homeNames = ["HOME 01", "HOME 02", "HOME 03", "HOME 04", "HOME 05", "HOME 06", "HOME 07", "HOME 08"];
    const homeWallColors = [0xe8d5b0, 0xd5c0a0, 0xe8d0b8, 0xd0b890, 0xe0cfa8, 0xc8b090, 0xdad0b0, 0xe5cba0];
    const homeRoofColors = [0x8a3a3a, 0x6a3a5a, 0x5a4a3a, 0x7a3a3a, 0x8a4a3a, 0x6a4a5a, 0x7a5a3a, 0x8a3a4a];
    for (let i = 0; i < 8; i++) {
      const px = -1650 + (i % 4) * 260;
      const pz = -1400 + Math.floor(i / 4) * 260;
      buildSmartHome(px, pz, homeNames[i], homeWallColors[i], homeRoofColors[i], 0);
    }

    // ===== VERTICAL FARM TOWERS (far east, off-road) =====
    buildVerticalFarm(2900, -2400, 6);

    // ===== GREENHOUSE (separate area, off-road) =====
    buildGreenhouse(3400, -1800);
    ...
    function buildHelipad(x, z) {
      ...
      scene.add(g); board("EMERGENCY HELIPAD", x, 5, z + 130, 260, 14, 0xccaa22);
    ...
    buildLibrary(2800, 2800); buildHelipad(-2800, -1500); buildSmartParking(-2800, 1200);
    ...
    const occupiedSpots = [
      ...
      { x: 0, z: 0, r: 130 }, { x: 3600, z: -3600, r: 600 }, { x: -3600, z: -3600, r: 600 },
      { x: 3600, z: 3600, r: 600 }, { x: -1800, z: 1800, r: 400 },
      { x: -600, z: 600, r: 500 }, { x: -1600, z: 800, r: 260 }, { x: -3900, z: -2400, r: 350 },
      { x: -3600, z: -800, r: 400 }, { x: -3600, z: 800, r: 350 }, { x: 900, z: 1400, r: 250 },
      { x: 2900, z: -2400, r: 500 }, { x: 3400, z: -1800, r: 500 }, { x: -1400, z: -1500, r: 700 },
    ];
    ...
    // ===== TALL TOWERS (Society) =====
    const TOWER_COLORS = [0x5b9bd5, 0x4a90e2, 0x7bb3e0, 0x6ba3d9, 0x82c0e8, 0x5090d0, 0xc0629b, 0xd475a8, 0xb84f8a, 0xe087b6, 0xf5a623, 0xf7b955, 0xe89b3e, 0xffc866, 0x6cd4a0, 0x4ec39a, 0x8adbb8, 0x5bc08a, 0x9b7ad9, 0xaa8ae0, 0x8a66c9, 0xbaa0e8, 0xe8635c, 0xd94f4a, 0xf0776b];
    function tallTower(x, z, w, h, d, color) {
      ...
      const glassMatT = new THREE.MeshStandardMaterial({ color: 0x1a2a3a, emissive: 0x66ccff, emissiveIntensity: 0.7, metalness: 0.6, roughness: 0.2 });
      for (let f = 1; f <= 4; f++) {
        const fy = (h / 5) * f;
        const line = new THREE.Mesh(new THREE.BoxGeometry(w + 0.5, 0.6, d + 0.5), glassMatT);
    ...
    // ===== TRAFFIC LIGHTS (street, all intersections) =====
    ...
    function tl(x, z) {
      ...
      const light = (color, y) => {
        const m = new THREE.Mesh(new THREE.SphereGeometry(0.9, 12, 12), new THREE.MeshStandardMaterial({ color, emissive: color, emissiveIntensity: 0 }));
        m.position.set(0, y, 1.2); g.add(m);
        const halo = new THREE.Mesh(new THREE.SphereGeometry(1.8, 12, 12), new THREE.MeshBasicMaterial({ color, transparent: true, opacity: 0, blending: THREE.AdditiveBlending }));
    ...
    // ===== INTERSECTION CARS (with traffic light logic) =====
    ...
    spawnPeople(2900, 1000, 5, 180); spawnPeople(-1400, -1500, 12, 500);
    ...
    // ===== POWER ZONE (out of city, solar + wind + batteries) =====
    ...
    const solarPanelMat2 = new THREE.MeshStandardMaterial({ color: 0x082c4b, roughness: 0.18, metalness: 0.7, emissive: 0x063b62, emissiveIntensity: 0.7 });
    const solarPanelGeo2 = new THREE.BoxGeometry(1, 1, 1);
    function solarArray(x, z, w, d, rows, cols) {
      ...
      for (let r = 0; r < rows; r++) for (let c = 0; c < cols; c++) {
        const p = new THREE.Mesh(solarPanelGeo2, solarPanelMat2);
    ...
    batteryPositions.forEach((pos) => {
      const g = new THREE.Group(); g.position.set(pos[0], 6, pos[1]);
      const body = new THREE.Mesh(new THREE.BoxGeometry(30, 40, 40), mat(0x2a3a4a, 0.5, 0.6)); body.position.y = 20; g.add(body);
      const top = new THREE.Mesh(new THREE.BoxGeometry(28, 4, 38), mat(0x22ff9d, 0.4, 0.5)); top.position.y = 42; g.add(top);
      powerZone.add(g);
    ...
    const filtrationMachine = new THREE.Mesh(new THREE.BoxGeometry(170, 90, 170), mat(0x3a5a6a, 0.4, 0.5));
    filtrationMachine.position.set(0, 50, 0); filtZone.add(filtrationMachine);
    const filtMachineTop = new THREE.Mesh(new THREE.BoxGeometry(150, 20, 150), new THREE.MeshStandardMaterial({ color: 0x0a4a6a, emissive: 0x22cfff, emissiveIntensity: 0.8 }));
    filtMachineTop.position.set(0, 105, 0); filtZone.add(filtMachineTop);
    clickable.push({ object: filtrationMachine, type: "filtration", name: "Filtration Machine" });

    // Tanks
    const filtrationStageMeshes = []; s.filtrationStageMeshes = filtrationStageMeshes;
    const TANK_GRID = 3, TANK_SPACING = 75;
    const FILTRATION_STAGES = [
      { id: "wastewater", label: "WASTEWATER", color: 0x6b4a2f },
      { id: "primary", label: "PRIMARY", color: 0x8a7a4a },
      { id: "biological", label: "BIOLOGICAL", color: 0x4a8a5a },
      { id: "aeration", label: "AERATION", color: 0x4ac8e0 },
      { id: "clarification", label: "CLARIFICATION", color: 0x6ab0d0 },
      { id: "advanced", label: "ADVANCED", color: 0x3aa0d0 },
      { id: "uv", label: "UV PURIFICATION", color: 0x9a6aff },
      { id: "storage", label: "STORAGE", color: 0x22cfff },
      { id: "recycling", label: "RECYCLING", color: 0x2ecc71 },
    ];
    for (let i = 0; i < FILTRATION_STAGES.length; i++) {
      const row = Math.floor(i / TANK_GRID), col = i % TANK_GRID;
      const tx = (col - 1) * TANK_SPACING, tz = (row - 1) * TANK_SPACING + 250;
      const tankColor = FILTRATION_STAGES[i].color;
      const tankMat = new THREE.MeshStandardMaterial({ color: tankColor, emissive: tankColor, emissiveIntensity: 0.4, metalness: 0.3, roughness: 0.3, transparent: true, opacity: 0.85 });
      const tankBody = new THREE.Mesh(new THREE.CylinderGeometry(22, 24, 40, 16), tankMat); tankBody.position.set(tx, 27, tz); filtZone.add(tankBody);
      const tankCap = new THREE.Mesh(new THREE.SphereGeometry(22, 16, 8, 0, Math.PI * 2, 0, Math.PI / 2), tankMat); tankCap.position.set(tx, 47, tz); filtZone.add(tankCap);
      ...
      const waterFill = new THREE.Mesh(new THREE.CylinderGeometry(19, 19, 1, 16), waterMat); waterFill.position.set(tx, 13, tz); filtZone.add(waterFill);
      filtrationStageMeshes.push({ tankBody, tankCap, waterFill, baseY: 13 });
    }
    ...
    s.filtrationCleanReservoir = { resWater };
    ...
    for (let i = 0; i < 3; i++) {
      ...
      ring.rotation.x = Math.PI / 2; ring.position.set(0, 7 + i * 2, 0); ring.userData = { phase: i * 0.4 };
    ...
    for (let row = 0; row < 3; row++) for (let col = 0; col < 4; col++) {
      const g = new THREE.Group(); g.position.set(-280 + col * 190, 5, -300 + row * 220);
    ...
    function animate() {
      ...
      // Intersection traffic lights (bright blink)
      ...
      // Rooftop home turbines — spin fast
      for (const bl of s.rooftopTurbines) bl.rotation.z = t * 3.5;

      // Radar + controller rings
      ...
      // ===== VERTICAL FARM CROP GROWTH (REAL-TIME) =====
      for (const vf of s.verticalFarms) {
        vf.beacon.material.emissiveIntensity = 4.5 + Math.sin(t * 4) * 2.5;
        vf.topLED.rotation.z = t * 0.6;
        for (const level of vf.cropLevels) {
          for (const c of level) {
            // Growth cycle: 0 → 1 → 0 loop
            const growCycle = ((t * 0.25) + c.grow) % 1;
            // Leaf scale: 0.3 to 1.3
            const leafScale = 0.5 + Math.sin(growCycle * Math.PI) * 0.9;
            c.leaves.scale.setScalar(leafScale);
            // Fruit grows and changes color
            const fruitScale = 0.3 + growCycle * 1.0;
            c.fruit.scale.setScalar(fruitScale);
            // Color transition: green → yellow → red → green
            const hue = growCycle < 0.5 ? 0.33 - growCycle * 0.4 : 0.13 - (growCycle - 0.5) * 0.4;
            const color = new THREE.Color().setHSL(Math.max(0, hue), 0.85, 0.5);
            c.fruit.material.color.copy(color);
            c.fruit.material.emissive.copy(color).multiplyScalar(0.3);
            // Stem grows
            c.stem.scale.y = 0.6 + growCycle * 0.8;
            c.stem.position.y = (0.6 + growCycle * 0.8) * 2;
          }
        }
      }

      // ===== GREENHOUSE CROP GROWTH (REAL-TIME) =====
      for (const bed of s.greenhouseCrops) {
        for (const c of bed) {
          const growCycle = ((t * 0.3) + c.grow) % 1;
          const leafScale = 0.6 + Math.sin(growCycle * Math.PI) * 0.9;
          c.leaves.scale.setScalar(leafScale);
          const fruitScale = 0.4 + growCycle * 1.1;
          c.fruit.scale.setScalar(fruitScale);
          const hue = growCycle < 0.5 ? 0.33 - growCycle * 0.4 : 0.13 - (growCycle - 0.5) * 0.4;
          const color = new THREE.Color().setHSL(Math.max(0, hue), 0.85, 0.55);
          c.fruit.material.color.copy(color);
          c.fruit.material.emissive.copy(color).multiplyScalar(0.35);
          c.stem.scale.y = 0.5 + growCycle * 0.9;
        }
      }
      ...
      // Filtration tanks pulse
      for (const tm of filtrationStageMeshes) {
        const pulse = Math.sin(t * 2 + tm.tankBody.position.x * 0.1);
        tm.waterFill.material.emissiveIntensity = 1.6 + pulse * 0.6;
        tm.waterFill.scale.y = 12 + Math.sin(t * 2.5 + tm.tankBody.position.x * 0.1) * 6;
        tm.waterFill.position.y = tm.baseY + tm.waterFill.scale.y / 2;
      }
      if (s.filtrationCleanReservoir) {
        const isFilling = Math.sin(t * 0.5) > 0;
    ...
    <>
      ...
      {securityHUD && (
        <div style={{ position: "fixed", top: 24, right: 24, background: "linear-gradient(135deg, rgba(6,20,35,0.92), rgba(12,35,55,0.9))", border: "1px solid rgba(34,207,255,0.5)", borderRadius: 14, padding: "14px 18px", color: "#fff", fontFamily: "system-ui, -apple-system, sans-serif", boxShadow: "0 0 30px rgba(34,207,255,0.35)", zIndex: 9997, minWidth: 260, backdropFilter: "blur(10px)", fontSize: 12 }}>
          <div style={{ fontSize: 11, letterSpacing: 2, color: "#7fe3ff", textTransform: "uppercase", marginBottom: 8, fontWeight: 700 }}>AI TRAFFIC and SECURITY</div>
