...
function createAITrafficSystem(callbacks) {
  const PHASE_DURATION = 8, YELLOW_DURATION = 2.5;
  ...
  function tick(delta) {
    ...
    callbacks.onTrafficUpdate?.({ phase, inYellow, currentGreenRoads, currentRedRoads, phaseProgress: inYellow ? yellowElapsed / YELLOW_DURATION : elapsed / PHASE_DURATION, stats: { ...stats }, timeInPhase: inYellow ? yellowElapsed : elapsed });
  }
...
function createAISecuritySystem(callbacks) {
  ...
  const gates = SECURITY_GATES_DEF.map((g) => ({ ...g, status: "OPEN", aiState: "NORMAL", armAngle: BARRIER_OPEN_ANGLE, targetAngle: BARRIER_OPEN_ANGLE, vehiclesDetected: 0, vehiclesWaiting: 0, density: "LOW" }));
  function tick(delta, vehicleSnapshot) {
    ...
    callbacks.onSecurityUpdate?.({ phase, activeGateId: activeGate.id, gates: gates.map((g) => ({ id: g.id, label: g.label, status: g.status, aiState: g.aiState, vehiclesDetected: g.vehiclesDetected, vehiclesWaiting: g.vehiclesWaiting, density: g.density, armAngle: g.armAngle, axis: g.axis, sign: g.sign, offset: g.offset, stop: g.stop })) });
  }
...
function createCitySimulation(callbacks) {
  ...
  function setState(s) {
    ...
    else if (s === 1) { t.state = "TRAFFIC JAM"; t.stateColor = "#ffd15a"; t.reason = "QUEUE BUILDING AT CENTRAL INTERSECTION"; t.level = "HEAVY"; t.flow = "28%"; t.mode = "ANALYZING"; t.incident = "<span>INCIDENT:</span> Cars queuing"; aiReason = { visible: true, title: "🔴 PROBLEM DETECTED", text: "<strong>REASON:</strong> Too many cars at central intersection.", result: "AI is <strong>reading the queue</strong> to figure out the best fix." }; }
    else if (s === 2) { t.state = "AI REROUTING"; t.stateColor = "#67e4ff"; t.reason = "AI SPLITTING TRAFFIC TO INNER ROADS"; t.level = "RECOVERING"; t.flow = "68%"; t.mode = "OPTIMIZING"; t.incident = "<span>AI ACTION:</span> Rerouting cars via inner roads"; aiReason = { visible: true, title: "🔵 AI IS ACTING", text: "<strong>AI DID THIS:</strong> Flipped signals to green on inner roads and redirected half the cars.", result: "Result: <strong>less cars at intersection → jam clearing.</strong>" }; }
    else if (s === 3) { t.state = "TRAFFIC CLEAR"; t.stateColor = "#6aff9d"; t.reason = "ALL ROUTES FLOWING NORMALLY"; t.level = "CLEAR"; t.flow = "96%"; t.mode = "OPTIMAL"; t.incident = "<span>SYSTEM:</span> All routes flowing normally"; aiReason = { visible: true, title: "🟢 RESOLVED BY AI", text: "<strong>AI RESOLVED IT</strong> by splitting traffic between outer and inner roads.", result: "Jam cleared in <strong>15 seconds</strong>." }; }
...
const SmartCity3D = forwardRef((props, ref) => {
  ...
  function setDayNight(night) {
    ...
    if (night) { s.scene.background = NIGHT_BG.clone(); s.ambient.intensity = 0.5; s.sun.intensity = 0.35; }
    else { s.scene.background = DAY_BG.clone(); s.ambient.intensity = 2.8; s.sun.intensity = 2.6; }
