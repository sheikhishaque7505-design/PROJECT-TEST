...
export const LOCATIONS = {
  school: { label: "Beacon School System", icon: "🏫", type: "school", info: "American curriculum · 450 students · 32 teachers · Robotics lab" },
  hospital: { label: "Smart City Hospital", icon: "🏥", type: "hospital", info: "24/7 emergency · AI diagnosis · 8 ICU beds · 32 patients" },
  bank: { label: "Smart City State Bank", icon: "🏦", type: "bank", info: "Digital banking · 8 ATMs · 1,240 transactions/hour" },
  society: { label: "BSS Smart Society", icon: "🏘", type: "society", info: "12 towers · 2,400 residents · 82% solar powered" },
  farm: { label: "Smart Eco Farm", icon: "🌾", type: "farm", info: "IoT sensors · 3 drones · 94% crop health" },
  event: { label: "Liverpool Event Hall", icon: "🎪", type: "event", info: "Events · Concerts · Capacity 2,000" },
  gas: { label: "Gas Station · Car Wash", icon: "🚗", type: "gas", info: "4 EV chargers · 6 fuel pumps" },
  sewage: { label: "Sewage & Gas Co.", icon: "🏭", type: "sewage", info: "Biogas 2.1 MW · 8M L/day processed" },
  culture: { label: "Culture Center", icon: "🏛", type: "culture", info: "Museums · VR tours · 45 exhibits" },
  powerCompany: { label: "City Power Supply Co.", icon: "🔌", type: "powerCompany", info: "Grid monitoring · 68 MW load" },
  scifi: { label: "Sci-Fi Building", icon: "🛸", type: "scifi", info: "Futuristic R&D · Quantum computing" },
  tower: { label: "Beautiful Tower", icon: "🗼", type: "tower", info: "320m landmark · Observation deck" },
  traffic: { label: "AI Traffic Controller", icon: "🚦", type: "traffic", info: "4 roads · Adaptive signals · 24 sensors" },
  power: { label: "Power Supply Zone", icon: "⚡", type: "power", info: "Solar 42 MW + Wind 28 MW = 70 MW" },
  filtration: { label: "Filtration System", icon: "💧", type: "filtration", info: "9-stage · 12M L/day · 99.7% purity" },
  food: { label: "AI Food Production", icon: "🍎", type: "food", info: "9 fields · Drones · 48 daily deliveries" },
  waste: { label: "Waste Management", icon: "♻️", type: "waste", info: "Recycling · Biogas 4.2 MW · 68% recycled" },
};
...
const POWER_STAGES = [
  { id: "solar", label: "Solar Array", color: 0xffcc22, duration: 5, desc: "42 MW" },
  { id: "wind", label: "Wind Turbines", color: 0x22cfff, duration: 5, desc: "28 MW" },
  { id: "battery", label: "Battery Storage", color: 0x22ff9d, duration: 5, desc: "78% charge" },
  { id: "grid", label: "Grid Distribution", color: 0xff6b6b, duration: 5, desc: "Stable" },
  { id: "load", label: "City Load Balance", color: 0xb266ff, duration: 5, desc: "68 MW load" },
];

const FOOD_STAGES = [
  { id: "planting", label: "Smart Planting", color: 0x2ecc71, duration: 5, desc: "9 fields" },
  { id: "irrigation", label: "Smart Irrigation", color: 0x22cfff, duration: 5, desc: "Sprinklers" },
  { id: "monitoring", label: "Drone Monitoring", color: 0xffcc22, duration: 5, desc: "3 drones" },
  { id: "harvest", label: "Robotic Harvest", color: 0xe67e22, duration: 5, desc: "Auto collect" },
  { id: "processing", label: "Food Processing", color: 0x9b59b6, duration: 5, desc: "Cut · Pack" },
  { id: "distribution", label: "City Distribution", color: 0xff6b6b, duration: 5, desc: "48 daily" },
];

const WASTE_STAGES = [
  { id: "collection", label: "Waste Collection", color: 0x8a6a3a, duration: 5, desc: "Bins collecting" },
  { id: "segregation", label: "Segregation", color: 0x3498db, duration: 5, desc: "4 streams" },
  { id: "recycling", label: "Recycling", color: 0x2ecc71, duration: 5, desc: "Processing" },
  { id: "biogas", label: "Biogas Generation", color: 0x4a7a3a, duration: 5, desc: "4.2 MW" },
  { id: "composting", label: "Composting", color: 0x5a3a1a, duration: 5, desc: "Organic" },
  ...
  return {
    tick(delta) {
      ...
      if (inRed) {
        ...
        elapsed += delta;
        if (elapsed >= 8) { inYellow = true; yEl = 0; addLog("Yellow"); }
      }
      for (let r = 1; r <= 4; r++) {
...
const SmartCity3D = forwardRef((props, ref) => {
  ...
  function goToSystem(k) {
    const P = {
      ...
      filtration: [[3600, 500, -2900], [3600, 5, -3600]],
      food: [[2900, 500, -400], [2900, 5, -1200]],
      waste: [[3600, 500, 4400], [3600, 5, 3600]],
    };
    if (P[k]) smoothCam(new THREE.Vector3(...P[k][0]), new THREE.Vector3(...P[k][1]), 1800);
  }
  function goToLocation(k) {
    const L = {
      school: [[-600, 400, -300], [-600, 5, -600]],
      hospital: [[600, 400, -300], [600, 5, -600]],
      bank: [[600, 400, 900], [600, 5, 600]],
      society: [[-300, 500, 900], [-600, 5, 600]],
      farm: [[1800, 400, -300], [1800, 5, -600]],
      event: [[600, 400, 2100], [600, 5, 1800]],
      gas: [[1800, 400, 2050], [1800, 5, 1750]],
      sewage: [[1800, 400, 900], [1800, 5, 600]],
      culture: [[-1800, 400, 2100], [-1800, 5, 1800]],
      powerCompany: [[-1600, 400, 1100], [-1600, 5, 800]],
      scifi: [[-3900, 400, -2100], [-3900, 5, -2400]],
      tower: [[-3600, 400, -500], [-3600, 5, -800]],
    };
    if (L[k]) smoothCam(new THREE.Vector3(...L[k][0]), new THREE.Vector3(...L[k][1]), 1800);
  }
  ...
  useEffect(() => {
    ...
    c.appendChild(renderer.domElement); s.renderer = renderer;
    ...
    const ambient = new THREE.HemisphereLight(0xffffff, 0x4a7a56, 2.8); scene.add(ambient); s.ambient = ambient;
    const sun = new THREE.DirectionalLight(0xffffff, 2.6); sun.position.set(-800, 1400, 500); scene.add(sun); s.sun = sun;
    const fill = new THREE.DirectionalLight(0xd0e8ff, 1.2); fill.position.set(800, 1000, -500); scene.add(fill);

    const mat = (c, r = 0.8, m = 0) => new THREE.MeshStandardMaterial({ color: c, roughness: r, metalness: m });
    const grassMat = mat(0x4d8f50, 0.98); const roadMat = mat(0x2a3238, 0.96); const roadLaneMat = mat(0x1e2428, 0.96);
    const yellowLine = mat(0xf5c84b, 0.65); const whiteLine = mat(0xffffff, 0.65);
    const darkMat = mat(0x182327, 0.65, 0.15); const sidewalkMat = mat(0x8a8f94, 0.9);
    ...
    [[CITY_HALF * 2 + 20, 3, 20, 0, 6, CITY_HALF], [CITY_HALF * 2 + 20, 3, 20, 0, 6, -CITY_HALF],
     [20, 3, CITY_HALF * 2 + 20, -CITY_HALF, 6, 0], [20, 3, CITY_HALF * 2 + 20, CITY_HALF, 6, 0]].forEach(([w, h, d, x, y, z]) => {
      const m = new THREE.Mesh(new THREE.BoxGeometry(w, h, d), boundMat); m.position.set(x, y, z); scene.add(m);
    });
    ...
    roadZs.forEach(roadE); roadXs.forEach(roadN);
    roadZs.forEach(z => {
    ...
    roadZs.forEach(z => { for (let x = -ROAD_HALF_LEN + 100; x <= ROAD_HALF_LEN - 100; x += 400) { streetLight(x, z + ROAD_HALF + 18); streetLight(x, z - ROAD_HALF - 18); } });
    ...
    function trafficLight(x, z, road, faceAngle) {
      ...
      const r = light(0xff2222, 26.5); const y = light(0xffcc22, 23); const gr = light(0x22ff66, 19.5);
      scene.add(g);
    ...
    for (let i = 0; i < 8; i++) {
      ...
      const b = new THREE.Mesh(new THREE.SphereGeometry(2, 16, 16), bMat); b.position.set(Math.cos(a) * 60, 8, Math.sin(a) * 60); tower.add(b);
    ...
    for (let i = 0; i < 10; i++) {
      ...
      const w = new THREE.Mesh(new THREE.BoxGeometry(5, 7, 0.6), wMat); w.position.set(Math.cos(a) * 24, 64, Math.sin(a) * 24); w.rotation.y = -a; tower.add(w);
      s.borderLights.push(wMat);
    }
    s.radar = new THREE.Mesh(new THREE.TorusGeometry(12, 1, 10, 32), new THREE.MeshStandardMaterial({ color: 0x61e7ff, emissive: 0x23dfff, emissiveIntensity: 3 })); s.radar.rotation.x = Math.PI / 2; s.radar.position.y = 80; tower.add(s.radar);
    s.controllerSig = new THREE.Mesh(new THREE.SphereGeometry(3.5, 20, 20), new THREE.MeshStandardMaterial({ color: 0x66e5ff, emissive: 0x33dfff, emissiveIntensity: 5 })); s.controllerSig.position.y = 95; tower.add(s.controllerSig);
    scene.add(tower);
    ...
    const occupied = [
      ...
      { x: 3600, z: -3600, r: 700 }, { x: 3600, z: 3600, r: 700 }, { x: 2900, z: -1200, r: 550 },
    ...
    // SMALL BUILDINGS
    function smallBld(x, z, w, h, d, color) {
    ...
    smallBld(-800, -300, 30, 20, 30, 0x8e44ad); smallBld(800, -300, 30, 20, 30, 0xf39c12);
    smallBld(-800, 300, 30, 20, 30, 0x3498db); smallBld(800, 300, 30, 20, 30, 0xe74c3c);
    smallBld(-1500, -900, 40, 25, 40, 0x9b59b6); smallBld(1500, -900, 40, 25, 40, 0x16a085);
    smallBld(-1500, 900, 40, 25, 40, 0x2ecc71); smallBld(1500, 900, 40, 25, 40, 0xd35400);
    smallBld(-2100, -500, 45, 30, 45, 0x34495e); smallBld(2100, -500, 45, 30, 45, 0x8e44ad);
    smallBld(-2100, 500, 45, 30, 45, 0xe67e22); smallBld(2100, 500, 45, 30, 45, 0x2980b9);
    smallBld(-2700, -2700, 50, 35, 50, 0x5b9bd5); smallBld(2700, -2700, 50, 35, 50, 0xc0629b);
    smallBld(-2700, 2700, 50, 35, 50, 0xf5a623); smallBld(2700, 2700, 50, 35, 50, 0x6cd4a0);
    ...
    function prep(o, size) {
      ...
      const max = Math.max(sz.x, sz.y, sz.z); o.scale.setScalar(size / Math.max(max, 0.001));
    ...
    function addBorder(pos, size, col) {
      const m = new THREE.MeshStandardMaterial({ color: col, emissive: col, emissiveIntensity: 2.5, metalness: 0.7, roughness: 0.2 });
      s.borderLights.push(m);
      const fw = size * 1.4;
      [[fw, 2.5, 6, pos[0], 6.5, pos[1] + fw / 2 + 10], [fw, 2.5, 6, pos[0], 6.5, pos[1] - fw / 2 - 10],
       [6, 2.5, fw, pos[0] - fw / 2 - 10, 6.5, pos[1]], [6, 2.5, fw, pos[0] + fw / 2 + 10, 6.5, pos[1]]].forEach(([w, h, d, x, y, z]) => {
        const b = new THREE.Mesh(new THREE.BoxGeometry(w, h, d), m); b.position.set(x, y, z); scene.add(b);
      });
    }
    function bld(file, size, pos, type, name, col) {
      const fallback = () => {
        const g = new THREE.Group(); const h = size * 0.8;
        const b = new THREE.Mesh(new THREE.BoxGeometry(size, h, size), mat(col, 0.6, 0.4)); b.position.set(pos[0], 5 + h / 2, pos[1]); g.add(b);
        const wMat = new THREE.MeshStandardMaterial({ color: 0x22cfff, emissive: 0x22cfff, emissiveIntensity: 0.8 });
        for (let i = 0; i < 4; i++) { const band = new THREE.Mesh(new THREE.BoxGeometry(size + 1, 2, size + 1), wMat); band.position.set(pos[0], 5 + h * (i + 1) / 5, pos[1]); g.add(band); }
        const cap = new THREE.Mesh(new THREE.BoxGeometry(size * 0.9, 4, size * 0.9), mat(0x1a1a1a, 0.5, 0.6)); cap.position.set(pos[0], 5 + h + 2, pos[1]); g.add(cap);
        scene.add(g); s.clickable.push({ object: g, type, name }); addBorder(pos, size, col);
      };
      loader.load(file, (g) => {
        const b = g.scene; prep(b, size); b.position.set(pos[0], 5, pos[1]); scene.add(b);
        s.clickable.push({ object: b, type, name }); addBorder(pos, size, col);
      }, undefined, fallback);
    }
    ...
    // SOCIETY
    const innerG = new THREE.Mesh(new THREE.BoxGeometry(820, 0.6, 820), mat(0xb8bcc0, 0.92)); innerG.position.set(-600, 4.85, 600); scene.add(innerG);
    const socGroup = new THREE.Group(); socGroup.position.set(-600, 0, 600); scene.add(socGroup);
    const towerCols = [0x5b9bd5, 0x7bb3e0, 0xc0629b, 0xf5a623, 0x6cd4a0, 0x9b7ad9, 0xe8635c];
    for (let r = 0; r < 4; r++) for (let cc = 0; cc < 3; cc++) {
      const tx = -340 + (cc + 0.5) * (680 / 3), tz = -340 + (r + 0.5) * (680 / 4), h = 180 + Math.random() * 180;
      const b = new THREE.Mesh(new THREE.BoxGeometry(30, h, 30), mat(towerCols[Math.floor(Math.random() * towerCols.length)], 0.4, 0.55));
      b.position.set(tx, 5 + h / 2, tz); socGroup.add(b);
    }
    ...
    function board(text, x, y, z, w = 200, h = 16, col = 0x22cfff) {
      ...
      const pole = new THREE.Mesh(new THREE.CylinderGeometry(0.5, 0.7, 14, 6), darkMat); pole.position.y = 7; g.add(pole);
      const b = new THREE.Mesh(new THREE.BoxGeometry(w, h, 0.9), mat(col, 0.35, 0.25)); b.position.y = 12; g.add(b);
      const cv = document.createElement("canvas"); cv.width = 1400; cv.height = 350;
      const ctx = cv.getContext("2d");
      ctx.fillStyle = "#" + col.toString(16).padStart(6, "0"); ctx.fillRect(0, 0, 1400, 350);
      ctx.strokeStyle = "#78e9ff"; ctx.lineWidth = 12; ctx.strokeRect(10, 10, 1380, 330);
      ctx.fillStyle = "#fff"; ctx.font = "bold 82px Arial"; ctx.textAlign = "center"; ctx.textBaseline = "middle";
    ...
    board("WATER FILTRATION", 3600, 5, -2900, 460, 20, 0x22cfff);
    board("AI FOOD PRODUCTION", 2900, 5, -400, 420, 22, 0x2ecc71);
    board("WASTE MANAGEMENT", 3600, 5, 4400, 420, 22, 0x2ecc71);
    ...
    const pZone = new THREE.Group(); pZone.position.set(-5400, 0, 3600); scene.add(pZone);
    const pPad = new THREE.Mesh(new THREE.BoxGeometry(1700, 3, 1400), mat(0x1d3a2e, 0.92)); pPad.position.set(0, 4, 0); pZone.add(pPad);
    ...
    [[1700, 3, 10, 0, 7, -700], [1700, 3, 10, 0, 7, 700], [10, 3, 1400, -850, 7, 0], [10, 3, 1400, 850, 7, 0]].forEach(([w, h, d, x, y, z]) => {
      const m = new THREE.Mesh(new THREE.BoxGeometry(w, h, d), pBMat); m.position.set(x, y, z); pZone.add(m);
    });
    for (const cx of [-850, 850]) for (const cz of [-700, 700]) {
      const post = new THREE.Mesh(new THREE.CylinderGeometry(6, 6, 40, 12), pBMat); post.position.set(cx, 25, cz); pZone.add(post);
      const capM = new THREE.MeshStandardMaterial({ color: 0xffffff, emissive: 0xffcc22, emissiveIntensity: 6 });
      const cap = new THREE.Mesh(new THREE.SphereGeometry(7, 16, 16), capM); cap.position.set(cx, 48, cz); pZone.add(cap); s.borderLights.push(capM);
    }
    ...
    s.turbines = [];
    const tMat = mat(0xf0f0f0, 0.4, 0.3), tGeo = new THREE.CylinderGeometry(1.2, 2, 80, 8), blGeo = new THREE.BoxGeometry(2.5, 32, 1);
    for (let row = 0; row < 4; row++) for (let col = 0; col < 3; col++) {
      const g = new THREE.Group(); g.position.set(-700 + col * 240, 6, -550 + row * 380); g.scale.setScalar(1 + (row % 3) * 0.08);
      const t = new THREE.Mesh(tGeo, tMat); t.position.y = 40; g.add(t);
      const blades = new THREE.Group(); blades.position.y = 80;
      for (let i = 0; i < 3; i++) { const b = new THREE.Mesh(blGeo, tMat); b.position.y = 15; b.rotation.z = i * Math.PI * 2 / 3; blades.add(b); }
    ...
    const sPark = new THREE.Group(); sPark.position.set(500, 6, 0); pZone.add(sPark);
    const sGround = new THREE.Mesh(new THREE.BoxGeometry(650, 2, 1300), mat(0x2a4536, 0.92)); sGround.position.y = 0; sPark.add(sGround);
    for (let i = 0; i < 5; i++) for (const xo of [-140, 140]) {
      const g = new THREE.Group(); g.position.set(xo, 4, -480 + i * 220); g.rotation.x = -0.22;
      for (let r = 0; r < 2; r++) for (let cc = 0; cc < 4; cc++) {
        const p = new THREE.Mesh(new THREE.BoxGeometry(60, 0.7, 85), sPMat); p.position.set((cc - 1.5) * 70, 0, (r - 0.5) * 90); g.add(p);
      }
      sPark.add(g);
    }
    s.batteryRings = []; s.powerRings = [];
    [[-500, 850], [-370, 850], [-240, 850], [-110, 850], [20, 850], [-500, 980], [-370, 980], [-240, 980], [-110, 980], [20, 980]].forEach(([x, z]) => {
      const b = new THREE.Mesh(new THREE.BoxGeometry(45, 60, 45), mat(0x2a5a4a, 0.4, 0.5)); b.position.set(x, 36, z); pZone.add(b);
      const rM = new THREE.MeshStandardMaterial({ color: 0x22ff9d, emissive: 0x22ff9d, emissiveIntensity: 2.5 });
      const r = new THREE.Mesh(new THREE.TorusGeometry(32, 1, 8, 20), rM); r.rotation.x = Math.PI / 2; r.position.set(x, 9, z); pZone.add(r);
      s.batteryRings.push(rM); s.powerRings.push(r);
    });

    // ═══════════════ FILTRATION ZONE ═══════════════
    const fZone = new THREE.Group(); fZone.position.set(3600, 5, -3600); scene.add(fZone);
    const fPad = new THREE.Mesh(new THREE.BoxGeometry(1300, 2, 1300), mat(0x1a2836, 0.95)); fPad.position.y = -0.5; fZone.add(fPad);
    ...
    [[1300, 3, 10, 0, 2, -650], [1300, 3, 10, 0, 2, 650], [10, 3, 1300, -650, 2, 0], [10, 3, 1300, 650, 2, 0]].forEach(([w, h, d, x, y, z]) => {
      const m = new THREE.Mesh(new THREE.BoxGeometry(w, h, d), fBMat); m.position.set(x, y, z); fZone.add(m);
    });
    const fHit = new THREE.Mesh(new THREE.PlaneGeometry(1300, 1300), new THREE.MeshBasicMaterial({ visible: false }));
    fHit.rotation.x = -Math.PI / 2; fHit.position.set(0, 1, 0); fZone.add(fHit);
    s.clickable.push({ object: fHit, type: "filtration", name: "Water Filtration System" });

    loader.load("/skid_filtration_system.glb", (g) => {
      const m = g.scene; prep(m, 260); m.position.set(300, 5, 0); fZone.add(m);
      s.clickable.push({ object: m, type: "filtration", name: "Skid Filtration Machine" });
    }, undefined, () => {
      const g = new THREE.Group(); g.position.set(300, 0, 0);
      const base = new THREE.Mesh(new THREE.BoxGeometry(90, 12, 170), mat(0x2c3e50, 0.5, 0.6)); base.position.y = 6; g.add(base);
      for (let i = 0; i < 4; i++) { const t = new THREE.Mesh(new THREE.CylinderGeometry(14, 14, 45, 12), mat(0x22cfff, 0.3, 0.5)); t.position.set(-30 + i * 20, 36, -50 + i * 30); g.add(t); }
      fZone.add(g);
    });
    s.filtTanks = [];
    for (let i = 0; i < FILTRATION_STAGES.length; i++) {
      const row = Math.floor(i / 3), col = i % 3;
      const tx = -470 + col * 100, tz = -320 + row * 100;
      ...
      const tb = new THREE.Mesh(new THREE.CylinderGeometry(25, 27, 50, 20), tMat); tb.position.set(tx, 25, tz); fZone.add(tb);
      const tc = new THREE.Mesh(new THREE.SphereGeometry(25, 20, 10, 0, Math.PI * 2, 0, Math.PI / 2), tMat); tc.position.set(tx, 50, tz); fZone.add(tc);
      ...
      const wf = new THREE.Mesh(new THREE.CylinderGeometry(22, 22, 1, 20), wMat); wf.position.set(tx, 10, tz); fZone.add(wf);
      s.filtTanks.push({ tb, tc, wf, baseY: 10 });
    }
    const res = new THREE.Mesh(new THREE.CylinderGeometry(60, 65, 55, 32), new THREE.MeshStandardMaterial({ color: 0x0a4a6a, roughness: 0.3, metalness: 0.5, emissive: 0x0a2a4a, emissiveIntensity: 1 }));
    res.position.set(-470, 27.5, 320); fZone.add(res);

    // ═══════════════ FOOD ZONE ═══════════════
    const foZone = new THREE.Group(); foZone.position.set(2900, 5, -1200); scene.add(foZone);
    const foPad = new THREE.Mesh(new THREE.BoxGeometry(1100, 2, 1100), mat(0x1a2e1e, 0.95)); foPad.position.y = -0.5; foZone.add(foPad);
    ...
    [[1100, 3, 10, 0, 2, -550], [1100, 3, 10, 0, 2, 550], [10, 3, 1100, -550, 2, 0], [10, 3, 1100, 550, 2, 0]].forEach(([w, h, d, x, y, z]) => {
      const m = new THREE.Mesh(new THREE.BoxGeometry(w, h, d), foBMat); m.position.set(x, y, z); foZone.add(m);
    });
    const foHit = new THREE.Mesh(new THREE.PlaneGeometry(1100, 1100), new THREE.MeshBasicMaterial({ visible: false }));
    ...
    for (let r = 0; r < 3; r++) for (let cc = 0; cc < 3; cc++) {
      const px = -320 + cc * 140, pz = -320 + r * 140;
      const plot = new THREE.Mesh(new THREE.BoxGeometry(120, 3, 120), mat(0x4a2f1a, 0.95)); plot.position.set(px, 1.5, pz); foZone.add(plot);
      const pBorderM = new THREE.MeshStandardMaterial({ color: 0x2ecc71, emissive: 0x2ecc71, emissiveIntensity: 1.5 });
      const pBorder = new THREE.Mesh(new THREE.BoxGeometry(124, 1, 124), pBorderM); pBorder.position.set(px, 3, pz); foZone.add(pBorder);
      for (let k = 0; k < 8; k++) {
        const cCol = cropCols[Math.floor(Math.random() * cropCols.length)];
        const crop = new THREE.Mesh(new THREE.ConeGeometry(4, 16, 6), new THREE.MeshStandardMaterial({ color: cCol, emissive: cCol, emissiveIntensity: 0.3, roughness: 0.8 }));
      ...
      const spBase = new THREE.Mesh(new THREE.CylinderGeometry(4, 5, 4, 8), mat(0x9aa0a6, 0.5, 0.7)); spBase.position.set(px, 5, pz); foZone.add(spBase);
      const sprayRing = new THREE.Mesh(new THREE.TorusGeometry(22, 0.5, 6, 24), new THREE.MeshStandardMaterial({ color: 0x22cfff, emissive: 0x22cfff, emissiveIntensity: 3, transparent: true, opacity: 0.7 }));
    ...
    const plant = new THREE.Group(); plant.position.set(0, 0, 150); foZone.add(plant);
    const pBody = new THREE.Mesh(new THREE.BoxGeometry(180, 120, 140), mat(0x1a8a4e, 0.4, 0.5)); pBody.position.y = 60; plant.add(pBody);
    const pRoof = new THREE.Mesh(new THREE.BoxGeometry(190, 6, 150), mat(0x0a3a1a, 0.5, 0.5)); pRoof.position.y = 123; plant.add(pRoof);
    s.foodBeacon = new THREE.Mesh(new THREE.SphereGeometry(7, 20, 20), new THREE.MeshStandardMaterial({ color: 0x2ecc71, emissive: 0x2ecc71, emissiveIntensity: 6 })); s.foodBeacon.position.y = 138; plant.add(s.foodBeacon);
    s.foodGears = [];
    for (let i = 0; i < 3; i++) { const g = new THREE.Mesh(new THREE.CylinderGeometry(8, 8, 3, 12), new THREE.MeshStandardMaterial({ color: 0x9aa0a6, metalness: 0.9, roughness: 0.2 })); g.rotation.x = Math.PI / 2; g.rotation.z = Math.PI / 2; g.position.set(-60 + i * 60, 30, 72); plant.add(g); s.foodGears.push(g); }
    s.foodConveyor = [];
    for (let i = 0; i < 8; i++) {
      const box = new THREE.Mesh(new THREE.BoxGeometry(18, 18, 18), new THREE.MeshStandardMaterial({ color: i % 2 ? 0xf39c12 : 0xe67e22, emissive: i % 2 ? 0xf39c12 : 0xe67e22, emissiveIntensity: 1.5 }));
      box.position.set(-110 + i * 30, 23, 200); box.userData = { startX: -110, endX: 110, offset: i * 30 }; plant.add(box); s.foodConveyor.push(box);
    }
    s.foodDrone = new THREE.Group(); s.foodDrone.position.set(-280, 80, -280); foZone.add(s.foodDrone);
    s.foodDrone.add(new THREE.Mesh(new THREE.BoxGeometry(12, 4, 12), mat(0x2c3e50, 0.5, 0.6)));
    s.foodDroneRotors = [];
    for (const [rx, rz] of [[-8, -8], [8, -8], [-8, 8], [8, 8]]) {
      const rot = new THREE.Mesh(new THREE.CylinderGeometry(5, 5, 1, 12), new THREE.MeshStandardMaterial({ color: 0xaaaaaa, metalness: 0.7, roughness: 0.3, transparent: true, opacity: 0.8 }));
      rot.position.set(rx, 3, rz); s.foodDrone.add(rot); s.foodDroneRotors.push(rot);
    }

    // ═══════════════ WASTE ZONE ═══════════════
    const wZone = new THREE.Group(); wZone.position.set(3600, 5, 3600); scene.add(wZone);
    const wPad = new THREE.Mesh(new THREE.BoxGeometry(1200, 2, 1200), mat(0x2a3a2e, 0.95)); wPad.position.y = -0.5; wZone.add(wPad);
    ...
    [[1200, 3, 10, 0, 2, -600], [1200, 3, 10, 0, 2, 600], [10, 3, 1200, -600, 2, 0], [10, 3, 1200, 600, 2, 0]].forEach(([w, h, d, x, y, z]) => {
      const m = new THREE.Mesh(new THREE.BoxGeometry(w, h, d), wBMat); m.position.set(x, y, z); wZone.add(m);
    });
    const wHit = new THREE.Mesh(new THREE.PlaneGeometry(1200, 1200), new THREE.MeshBasicMaterial({ visible: false }));
    ...
    for (let r = 0; r < 3; r++) for (let cc = 0; cc < 3; cc++) {
      const px = -350 + cc * 100, pz = -350 + r * 100;
      const bCol = binCols[(r * 3 + cc) % 4];
      const bG = new THREE.Group(); bG.position.set(px, 5, pz);
      const body = new THREE.Mesh(new THREE.CylinderGeometry(18, 21, 36, 12), mat(bCol, 0.5, 0.4)); body.position.y = 18; bG.add(body);
      const lid = new THREE.Mesh(new THREE.CylinderGeometry(20, 20, 3, 12), mat(0x1a1a1a, 0.6, 0.5)); lid.position.y = 37; bG.add(lid);
      const ledM = new THREE.MeshStandardMaterial({ color: bCol, emissive: bCol, emissiveIntensity: 3 });
      const led = new THREE.Mesh(new THREE.TorusGeometry(19, 0.7, 6, 20), ledM); led.rotation.x = Math.PI / 2; led.position.y = 25; bG.add(led);
      const fM = new THREE.MeshStandardMaterial({ color: 0xffcc22, emissive: 0xffcc22, emissiveIntensity: 2 });
      const f = new THREE.Mesh(new THREE.CylinderGeometry(3, 3, 10, 8), fM); f.position.y = 46; bG.add(f);
      wZone.add(bG);
    ...
    for (let i = 0; i < 10; i++) {
      const it = new THREE.Mesh(new THREE.BoxGeometry(12, 12, 12), new THREE.MeshStandardMaterial({ color: wItemCols[i % 4], emissive: wItemCols[i % 4], emissiveIntensity: 1.5 }));
      it.position.set(-150 + i * 30, 26, 0); it.userData = { startX: -150, endX: 150, offset: i * 30, speed: 0.6 + Math.random() * 0.4 };
      wZone.add(it); s.wasteConveyor.push(it);
    }
    const recM = new THREE.Group(); recM.position.set(-380, 0, 300); wZone.add(recM);
    recM.add(new THREE.Mesh(new THREE.BoxGeometry(100, 90, 100), mat(0x2ecc71, 0.4, 0.5)));
    recM.children[0].position.y = 45;
    s.wasteGears = [];
    for (let i = 0; i < 3; i++) { const g = new THREE.Mesh(new THREE.CylinderGeometry(14, 14, 4, 8), new THREE.MeshStandardMaterial({ color: 0x9aa0a6, metalness: 0.9, roughness: 0.2 })); g.rotation.x = Math.PI / 2; g.position.set(-30 + i * 30, 50, 52); recM.add(g); s.wasteGears.push(g); }
    const bioG = new THREE.Group(); bioG.position.set(380, 0, 300); wZone.add(bioG);
    const dig = new THREE.Mesh(new THREE.SphereGeometry(55, 20, 16), mat(0x4a7a3a, 0.6, 0.3)); dig.position.y = 55; bioG.add(dig);
    s.wasteFlame = new THREE.Mesh(new THREE.ConeGeometry(8, 25, 8), new THREE.MeshStandardMaterial({ color: 0xff6600, emissive: 0xff6600, emissiveIntensity: 5, transparent: true, opacity: 0.9 }));
    s.wasteFlame.position.y = 145; bioG.add(s.wasteFlame);
    for (let i = 0; i < 3; i++) {
      const pit = new THREE.Mesh(new THREE.CylinderGeometry(35, 35, 12, 16), mat(0x5a3a1a, 0.9)); pit.position.set(-380 - 80 + i * 80, 6, -300); wZone.add(pit);
      const steam = new THREE.Mesh(new THREE.SphereGeometry(8, 8, 6), new THREE.MeshStandardMaterial({ color: 0xffffff, transparent: true, opacity: 0.35 }));
      steam.position.set(-380 - 80 + i * 80, 32, -300); wZone.add(steam); s.wasteSteam.push(steam);
    }
    for (let i = 0; i < 6; i++) {
      const sm = new THREE.Mesh(new THREE.SphereGeometry(8, 6, 6), new THREE.MeshStandardMaterial({ color: 0xcccccc, transparent: true, opacity: 0.4 }));
      sm.position.set(380 + (Math.random() - 0.5) * 15, 220 + i * 15, -300); wZone.add(sm); s.wasteSmoke.push(sm);
    }
    ...
    function makeCar(col) {
      const g = new THREE.Group(); const bM = mat(col, 0.35, 0.5);
    ...
    function updateCar(c) {
      const { lane, pos } = c; const lp = Math.round(pos * 10) / 10;
      if (lane.axis === "z") {
      ...
      c.car.rotation.x = 0; c.car.rotation.z = 0;
    }
    LANES.forEach(lane => {
      for (let i = 0; i < 3; i++) {
        const car = makeCar(carCols[Math.floor(Math.random() * carCols.length)]); scene.add(car);
    ...
    function spawnPeople(cx, cz, n, r) {
      for (let i = 0; i < n; i++) {
        ...
        g.add(new THREE.Mesh(new THREE.SphereGeometry(2, 8, 6), mat(skinC[Math.floor(Math.random() * skinC.length)], 0.9))).position.y = 10;
        const body = new THREE.Mesh(new THREE.BoxGeometry(3, 5.5, 2), mat(shirtC[Math.floor(Math.random() * shirtC.length)], 0.85));
        body.position.y = 6; g.add(body);
        const legL = new THREE.Mesh(new THREE.BoxGeometry(1.2, 4.5, 1.2), mat(0x2c3e50, 0.85)); legL.position.set(-0.8, 2.2, 0); g.add(legL);
        const legR = new THREE.Mesh(new THREE.BoxGeometry(1.2, 4.5, 1.2), mat(0x2c3e50, 0.85)); legR.position.set(0.8, 2.2, 0); g.add(legR);
        g.position.set(px, 5, pz); g.rotation.y = a;
        scene.add(g);
    ...
    // ═══════════════ AI SYSTEM ═══════════════
    s.aiSystem = createAITrafficSystem((data) => { if (s.onAITrafficUpdate) s.onAITrafficUpdate(data); });

    // ═══════════════ CLICK ═══════════════
    const ray = new THREE.Raycaster(); const mous = new THREE.Vector2();
    const onClick = (e) => {
    ...
    function animate() {
      ...
      try {
        ...
        // CARS
        const byLane = {};
        for (const c of s.cars) { if (!byLane[c.lane.id]) byLane[c.lane.id] = []; byLane[c.lane.id].push(c); }
        for (const laneId in byLane) {
          const lCars = byLane[laneId]; const lane = lCars[0].lane; const ai = s.aiSystem;
          lCars.sort((a, b) => lane.dir > 0 ? b.pos - a.pos : a.pos - b.pos);
          for (let i = 0; i < lCars.length; i++) {
            const c = lCars[i]; const leader = i > 0 ? lCars[i - 1] : null;
            let ts = c.baseSpeed;
            const isG = ai.isGreen(lane.road); const isY = ai.isYellowRoad(lane.road);
            const sl = lane.dir > 0 ? -130 : 130; const d = Math.abs(c.pos - sl);
            if (!isG && d < 220) { ts = d < 40 ? 0 : c.baseSpeed * (d / 220) * 0.5; }
            else if (isY && d < 120) ts = c.baseSpeed * 0.4;
            if (leader) { const gap = Math.abs(leader.pos - c.pos); if (gap < 90) { ts = Math.min(ts, leader.speed * 0.9); if (gap < 45) ts = 0; } }
        ...
        // LIGHTS
        const bF = Math.floor(t * 4) % 2 === 0; const bM = Math.floor(t * 2) % 2 === 0;
        for (const l of s.intLights) {
          ...
          l.rH.material.opacity = 0; l.yH.material.opacity = 0; l.grH.material.opacity = 0;
        ...
        if (s.controllerSig) { s.controllerSig.scale.setScalar(1 + Math.sin(t * 4) * 0.2); s.controllerSig.material.emissiveIntensity = 4 + Math.sin(t * 4) * 2; }
        ...
        if (fc % 15 === 0 && s.onFiltrationUpdate) s.onFiltrationUpdate({ stageIndex: s.fIdx, stage: FILTRATION_STAGES[s.fIdx], allStages: FILTRATION_STAGES, progress: s.fEl / FILTRATION_STAGES[s.fIdx].duration });
        for (let i = 0; i < s.filtTanks.length; i++) {
          const tk = s.filtTanks[i]; const isA = i === s.fIdx;
          if (isA) { tk.tb.material.emissiveIntensity = 1.5 + Math.sin(t * 4) * 0.8; const fh = 40 + Math.sin(t * 2.5) * 12; tk.wf.scale.y = fh; tk.wf.position.y = tk.baseY + fh / 2; }
          else { tk.tb.material.emissiveIntensity = 0.5; tk.wf.scale.y = 10; tk.wf.position.y = tk.baseY + 5; }
        }
        // POWER
        s.pEl += delta;
        if (s.pEl >= POWER_STAGES[s.pIdx].duration) { s.pEl = 0; s.pIdx = (s.pIdx + 1) % POWER_STAGES.length; }
        if (fc % 15 === 0 && s.onPowerUpdate) s.onPowerUpdate({ stageIndex: s.pIdx, stage: POWER_STAGES[s.pIdx], allStages: POWER_STAGES, progress: s.pEl / POWER_STAGES[s.pIdx].duration });
        // FOOD
        s.foEl += delta;
        if (s.foEl >= FOOD_STAGES[s.foIdx].duration) { s.foEl = 0; s.foIdx = (s.foIdx + 1) % FOOD_STAGES.length; }
        if (fc % 15 === 0 && s.onFoodUpdate) s.onFoodUpdate({ stageIndex: s.foIdx, stage: FOOD_STAGES[s.foIdx], allStages: FOOD_STAGES, progress: s.foEl / FOOD_STAGES[s.foIdx].duration });
        const fSid = FOOD_STAGES[s.foIdx].id;
        for (const c of s.foodCrops) { c.rotation.z = Math.sin(t * 1.5 + c.userData.phase) * 0.08; const gb = (fSid === "planting" || fSid === "irrigation") ? (1 + Math.sin(t * 0.8 + c.userData.phase) * 0.15) : 1; c.scale.y = c.userData.scale * gb; }
        for (const sp of s.foodSprinklers) { const ph = (t * 0.8 + sp.phase) % 2; sp.ring.scale.setScalar(1 + ph * 0.8); sp.ring.material.opacity = Math.max(0, 0.7 - ph * 0.35); sp.ring.visible = fSid === "irrigation" || fSid === "planting"; }
        const bs = fSid === "processing" ? 1.5 : 0.5;
        for (const it of s.foodConveyor) { it.position.x += bs * delta * 30; if (it.position.x > it.userData.endX) it.position.x = it.userData.startX; it.position.y = 23 + Math.sin(t * 5 + it.userData.offset) * 1.5; }
        const gs = fSid === "processing" ? 4 : 1.5;
        for (const g of s.foodGears) g.rotation.y += delta * gs;
        if (s.foodBeacon) { s.foodBeacon.scale.setScalar(1 + Math.sin(t * 3) * 0.3); s.foodBeacon.material.emissiveIntensity = 5 + Math.sin(t * 3) * 2; }
        if (s.foodDrone) { const da = t * 0.4; s.foodDrone.position.x = Math.cos(da) * 320; s.foodDrone.position.z = Math.sin(da) * 320; s.foodDrone.position.y = 80 + Math.sin(t * 1.5) * 10; s.foodDrone.rotation.y = -da + Math.PI / 2; for (const r of s.foodDroneRotors) r.rotation.y += delta * 30; }
        // WASTE
        s.wEl += delta;
        if (s.wEl >= WASTE_STAGES[s.wIdx].duration) { s.wEl = 0; s.wIdx = (s.wIdx + 1) % WASTE_STAGES.length; }
        if (fc % 15 === 0 && s.onWasteUpdate) s.onWasteUpdate({ stageIndex: s.wIdx, stage: WASTE_STAGES[s.wIdx], allStages: WASTE_STAGES, progress: s.wEl / WASTE_STAGES[s.wIdx].duration });
        for (const bin of s.wasteBins) bin.led.emissiveIntensity = 2.5 + Math.sin(t * 3 + bin.phase) * 1.5;
        for (const it of s.wasteConveyor) { it.position.x += it.userData.speed * delta * 30; if (it.position.x > it.userData.endX) it.position.x = it.userData.startX; it.position.y = 26 + Math.sin(t * 6 + it.userData.offset) * 2; it.rotation.y += delta * 2; }
        for (const g of s.wasteGears) g.rotation.y += delta * 3;
        if (s.wasteFlame) { s.wasteFlame.scale.y = 1 + Math.sin(t * 8) * 0.3; s.wasteFlame.material.emissiveIntensity = 4 + Math.sin(t * 8) * 2; }
        for (const st of s.wasteSteam) { st.position.y += delta * 5; st.material.opacity = Math.max(0, 0.35 - (st.position.y - 32) * 0.008); if (st.position.y > 55) st.position.y = 32; }
        for (const sm of s.wasteSmoke) { sm.position.y += delta * 8; sm.material.opacity = Math.max(0, 0.4 - (sm.position.y - 220) * 0.003); if (sm.position.y > 400) sm.position.y = 220; }
        // CAMERA
        if (s.camTransition && s.camera && s.controls) {
          const now = performance.now(); const el = now - s.camTransition.startTime; const tt = Math.min(el / s.camTransition.duration, 1);
